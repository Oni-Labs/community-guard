...
app.post("/auth/register", async (req, res) => {
  ...
  if (!email)
    return res.status(422).json({ message: "O email é obrigatório22" });
  if (!password)
...
mongoose
  .connect(
    `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@communitysurveillance.woeig.mongodb.net/?retryWrites=true&w=majority&appName=CommunitySurveillance`
  )
  .then(() => {
